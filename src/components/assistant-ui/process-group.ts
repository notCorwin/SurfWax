...
export function processGroupSummary(parts: readonly ProcessPart[], indices: readonly number[], interrupted = false) {
  ...
  if (active?.status.type === "requires-action" && !interrupted) return { status: "running" as const, label: "等待操作…", hasTools: tools.length > 0 };
  if (active?.status.type === "running" && active.type === "reasoning") return { status: "running" as const, label: "正在思考…", hasTools: tools.length > 0 };
  if (active?.status.type === "running" && active.type === "tool-call") {
    ...
    return { status: "running" as const, label: activity.label, hasTools: true };
  }
