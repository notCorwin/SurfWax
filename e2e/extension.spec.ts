...
test("manages, restores, runs and deletes scripts in a separate Chrome tab", async () => {
  ...
  try {
    ...
    await manager.locator("#script-code").fill(script.js[0].code);
    await manager.getByRole("button", { name: "保存" }).click();
    ...
    await manager.locator("#script-code").fill("document.documentElement.dataset.managed = 'updated'; 'RUN_OK'");
    ...
    await expect(manager.locator("#script-code")).toHaveValue(/updated/);
