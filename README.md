...
Side Panel 标题栏的脚本按钮会打开独立的用户脚本标签页。可搜索和选择已保存脚本，分别填写脚本 ID、网站匹配规则及 JavaScript 代码；新脚本的网站范围需要明确填写。高级编辑入口保留 Chrome 原生 `RegisteredUserScript` JSON 格式及全部字段。试运行会在明确选择的网页标签页执行当前编辑内容，无需先保存。
